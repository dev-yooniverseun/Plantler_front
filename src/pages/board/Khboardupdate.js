...
function Khboardupdate() {
    ...
    useEffect(() =>{
        ...
        .then(res=> {
            if(res.data.state) {
                console.log(res);
                setBoardUpdate(res.data.result);
                ref_title.current.value = res.data.result.title;
                ref_content.current.value = res.data.result.content;
                
                // 선택된 카테고리 설정
                document.getElementsByName("part").forEach((input) => {
                    if (input.value === res.data.result.category_id.toString()) {
                        input.checked = true;
                    }
                });
            } else {
    ...
    const submitEvent = () => {
        ...
        axios.post(`${process.env.REACT_APP_BACK_HOST_URL}/khboardupdate`, formData, { headers: {'Authorization': cookie.token, 'Content-Type': 'multipart/form-data'} })
