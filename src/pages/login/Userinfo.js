...
const Userinfo = () => {
    ...
    useEffect(() => {
        ...
        if (!token) {
            ...
            // 사용자 정보를 가져오는 API 요청
            axios.post("http://localhost:8080/api/getUserByNo", { token: token })
                .then(res => {
                    if (res.data) {
                        setUserInfo({
        ...
        axios.post("http://localhost:8080/api/getUserByNo", { token: token }) 
            .then(res => {
                if (res.data.user_nick === userNick) {
                    ...
                    // 닉네임 중복 체크
                    axios.post("http://localhost:8080/api/checkUserNick", { nick: userNick })
                        .then(res => {
                            if (res.data.exists) {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post("http://localhost:8080/api/edit", params)
        .then(res => {
            if (res.data.status) {
    ...
    const handledelete = (e) => {
        ...
        axios.post('http://localhost:8080/api/userdelete', { user_nick })
        .then((response) => {
